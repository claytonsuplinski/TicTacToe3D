...
//float time = Position.z + Position.x;
...
void main() {
    ...
    color *= 3.5;
	vec4 c = vec4(clamp(color, 0.05, 0.85),clamp(color, 0.05, 0.05),clamp(color, 0.05, 0.1),1.0); //lava
    //vec4 c = vec4(clamp(color, 0.05, 0.1),clamp(color, 0.35, 0.5),clamp(color, 0.05, 0.1),1.0);
    
     
    gl_FragColor = c*0.1 + vec4(ads()*0.35, 1.0);
}
